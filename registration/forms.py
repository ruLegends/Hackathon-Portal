...
class RegistrationForm(UserCreationForm):
    ...
    email = forms.EmailField(
       ...
       # Мои поля для регистрации пока что не связанные с таблицей



    #fam = forms.CharField(max_length=200, required=True)
    #im = forms.CharField(max_length=200, required=True)
    #nameuse = forms.CharField(max_length=200, required=True)
    #telefon = forms.CharField(max_length=200, required=True)
    #organizac = forms.CharField(max_length=200, required=True)
...
class RegistrationFormNoFreeEmail(RegistrationForm):
    ...
    def clean_email(self):
        """
        Check the supplied email address against a list of known freeregistration_registrationprofile_pkey
        webmail domains.
