...
class RegistrationManager(models.Manager):
    ...
    def create_inactive_user(self, form, site, send_email=True):
        ...
        #------------------------ИЗМЕНИТЬ ОБРАТНО ДЛЯ КОРРЕКТНОЙ РАБОТЫ МОДУЛЯ--------------
        #Было заменено значение с False на True для активации учетной записи без подтвержения из почты
        #для обратной активации проверки с высыланием письма с кодом вернуть new_user.is_active=False
        #(это свойство проставляет в учетной записи галочку в чекюоксе "неактивен")
...
class Employee(models.Model):
    ...
    fam = models.CharField(max_length=200, verbose_name=u"Фамилия")
    im = models.CharField(max_length=200, verbose_name=u"Имя")
    nameuse = models.CharField(max_length=200, verbose_name=u"Отчество")
    telefon = models.CharField(max_length=200, verbose_name=u"Номер телефона")
    organizac = models.CharField(max_length=200, verbose_name=u"Организация")
